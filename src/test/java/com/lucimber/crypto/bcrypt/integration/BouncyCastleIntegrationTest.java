...
/**
 * Integration tests comparing our BCrypt implementation with Bouncy Castle's implementation.
 * These tests ensure our implementation is compatible with a widely-used cryptographic library.
 */
class BouncyCastleIntegrationTest {
    ...
    void shouldGenerateHashCompatibleWithBouncyCastle() {
        ...
        boolean bcVerifies = BouncyCastleHelper.verifyWithBouncyCastle(passwordString, ourHash.getValue());
    ...
    void shouldVerifyBouncyCastleGeneratedHash() {
        ...
        assertTrue(service.verify(password, hash),
            "Should verify hash generated by Bouncy Castle");
        
        // Also verify that BC can verify its own hash
        assertTrue(BouncyCastleHelper.verifyWithBouncyCastle(passwordString, bcHash),
    ...
    void shouldHandleVariousCostFactorsLikeBouncyCastle() {
        ...
        for (int cost : costs) {
            ...
            assertTrue(service.verify(password, hash),
                "Should verify BC hash with cost " + cost);
            ...
            assertEquals(bcHash, ourHash.getValue(),
                "Hashes should match for cost " + cost);
        }
    }
    ...
    void shouldProduceDifferentHashesForDifferentSaltsLikeBouncyCastle() {
        ...
        assertNotEquals(ourHash1.getValue(), ourHash2.getValue(),
    ...
    void shouldHandleUtf8PasswordsLikeBouncyCastle() {
        ...
        for (String passwordString : passwords) {
            ...
            // Generate with our implementation
            Salt ourSalt = new Salt(salt);
            Hash ourHash = service.hash(password, BCryptVersion.VERSION_2A, new CostFactor(10), ourSalt);
    ...
    void shouldHandlePasswordTruncationLikeBouncyCastle() {
        ...
        String bcHashTruncated = BouncyCastleHelper.hashWithBouncyCastle(truncatedPassword, salt, 10);
        
        // BC should produce same hash for both (truncates at 72)
        assertEquals(bcHashTruncated, bcHashLong,
            "BC should truncate at 72 bytes");
        ...
        Hash hashLong = service.hash(longPwd, BCryptVersion.VERSION_2A, new CostFactor(10), ourSalt);
        Hash hashTrunc = service.hash(truncPwd, BCryptVersion.VERSION_2A, new CostFactor(10), ourSalt);
        
        assertEquals(hashTrunc.getValue(), hashLong.getValue(),
            "Our implementation should truncate at 72 bytes");
        
        // All should produce the same hash
        assertEquals(bcHashLong, hashLong.getValue(),
