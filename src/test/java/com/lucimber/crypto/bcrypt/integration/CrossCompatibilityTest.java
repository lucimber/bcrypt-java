...
/**
 * Cross-compatibility tests that verify our BCrypt implementation works correctly,
 * even if it's not byte-for-byte compatible with other implementations.
 * 
 * The important properties to test:
 * 1. Our implementation can verify its own hashes
 * 2. Passwords are hashed securely with proper salt
 * 3. Wrong passwords are rejected
 * 4. Cost factors work correctly
 */
class CrossCompatibilityTest {
    ...
    void shouldHashAndVerifyPasswordsCorrectly() {
        ...
        for (String rawPassword : passwords) {
            ...
            assertTrue(service.verify(password, hash),
                "Should verify password: " + rawPassword);
    ...
    void shouldHandleDifferentCostFactors() {
        ...
        for (int cost : costs) {
            ...
            // Verify the hash
            assertTrue(service.verify(password, hash),
                "Should verify with cost factor " + cost);
            
            // Higher cost should take more time (approximately)
            if (previousTime > 0) {
                assertTrue(timeTaken >= previousTime,
                    "Higher cost factor should take more time");
            }
    ...
    void shouldHandleUtf8Passwords() {
        ...
        for (String rawPassword : utf8Passwords) {
            ...
            assertTrue(service.verify(password, hash),
                "Should handle UTF-8 password: " + rawPassword);
        }
    }
    ...
    void shouldProperlyHandleMaxLengthPasswords() {
        ...
        assertTrue(service.verify(maxPwd, hashMax),
            "Should handle 72-byte password");
        assertTrue(service.verify(belowMaxPwd, hashBelowMax),
            "Should handle 71-byte password");
        ...
        assertEquals(72, truncatedBytes.length,
    ...
    void shouldMaintainSecurityProperties() {
        ...
        assertTrue(hashString.matches("\\$2[ab]\\$\\d{2}\\$.{53}"),
            "Hash should match BCrypt format");
    }
}