...
/**
 * Represents a cryptographic salt used in BCrypt hashing.
 * A salt is a random value that ensures the same password produces different hashes.
 * BCrypt uses a 128-bit (16-byte) salt.
 */
public final class Salt {
    ...
    public Salt(byte[] salt) {
        ...
        if (salt.length != SALT_LENGTH) {
            throw new IllegalArgumentException(
                String.format("Salt must be exactly %d bytes, but was %d", SALT_LENGTH, salt.length)
            );
        }
    ...
    /**
     * Creates a Salt from a BCrypt-encoded string.
     * BCrypt uses a custom Base64 encoding with a different character set.
     * 
