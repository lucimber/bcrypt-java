...
/**
 * Represents the cost factor (also known as rounds or work factor) for BCrypt hashing.
 * The cost factor determines how computationally expensive the hashing operation is.
 * Higher values provide better security but take longer to compute.
 * The actual number of rounds is 2^costFactor.
 */
public final class CostFactor {
    ...
    /**
     * Default cost factor for new hashes.
    ...
    public CostFactor(int value) {
        if (value < MIN_COST || value > MAX_COST) {
            throw new IllegalArgumentException(
                String.format("Cost factor must be between %d and %d, but was %d", 
